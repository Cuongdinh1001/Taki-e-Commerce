const discountByProduct = function (listOrderDetail, voucher) {
  ...
  if (voucher.typeDiscount === "direct") {
    ...
    if (voucher.requirement.product.maxDiscount && voucher.requirement.product.maxDiscount < valueDiscount)
      valueDiscount = voucher.requirement.product.maxDiscount;
  }
...
const discountByCategory = function (listOrderDetail, voucher) {
  ...
  listProductSameCategory.forEach(order => {
    ...
    let postPrice = order.postPrice;
    let valueDiscount = 0;

    if (postPrice - remaining < 0) {
      remaining = remaining - postPrice;
      valueDiscount = postPrice;
      postPrice = 0;
    }
    else {
      postPrice = postPrice - remaining;
...
const discountByOrder = function (listOrderDetail, voucher) {
  ...
  if (voucher.typeDiscount === "direct") remaining = voucher.valueDiscount;
    ...
    if (voucher.requirement.order.maxDiscount && voucher.requirement.order.maxDiscount < remaining) 
      remaining = voucher.requirement.order.maxDiscount;
  }
